...
verseChoose.addEventListener("change", function(){
    let selectedValue = verseChoose.value;
...
console.log("Recieved Value:" + selectedVerse);
let url = selectedVerse + ".txt"; // `${selectedVerse}.txt`
...
.catch(error => pre.textContent = "There has been an error in getting the data from the API: " + error)
